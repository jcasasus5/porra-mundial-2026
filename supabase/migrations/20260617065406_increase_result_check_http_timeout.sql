...
do $$
begin
  if exists (select 1 from public.app_settings where key = 'app_url')
    and exists (select 1 from public.app_settings where key = 'cron_secret') then
    perform public.schedule_all_match_result_checks();
  end if;
end;
$$;
